...
export default function Website() {
    ...
    const [todoList, setTodoList] = useState(() => {
        return JSON.parse(localStorage.getItem('websiteList')) || []
    }) //sets useState as information within localStorage : set as empty array

    useEffect(() => {
        if (todoList.length > 0) {
            localStorage.setItem('websiteList', JSON.stringify(todoList))
        }
    ...
    <div>
            ...
            {todoList.map((object) => (
                <div className='box' key={object.id}>
                    <Row>
                        ...
                        <Col xs={7} className="context" style={{color: "purple"}}>{object.task}</Col>
        ...
        <div className='input'>
            <form className='form' onSubmit={preventDefault}>
                <input id="inputBox" type="text" placeholder='Enter Website Issues' value={todo} onChange={(e) => {setTodo(e.target.value)}}/>
